# Contributing to Vizzy (formerly Canopy Charts)

Thanks for helping improve the project! This repo bundles three pieces:

1. **`_new/packages/charts/`** – the published `@canopy/charts` package (D3 renderers, hooks, shared UI, Jest tests).
...
3. **`cli/`** – the `create-canopy-charts` (soon `create-vizzy-charts`) CLI that scaffolds wrapper components and installs dependencies.
...
| `npm run test:charts`| Jest suite for `@canopy/charts`             |
...
- Import charts from `@canopy/charts`; the old `app/_components/charts` directory has been removed and is lint-restricted.
...
- CLI templates now live in `cli/templates/charts` and simply import from `@canopy/charts`.
...
- Run `npm run lint && npm run test:charts` (and `cli` tests if touched) so CI stays green.
